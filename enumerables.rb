...
def spiciest_foods(spicy_foods)
  spicy_foods.select {|food| food[:heat_level] if food[:heat_level] > 5}
end
...
def print_spiciest_foods(spicy_foods)
    spicy_foods.map {|food| puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{ "🌶" * food[:heat_level]}" if food[:heat_level] > 5}
end
